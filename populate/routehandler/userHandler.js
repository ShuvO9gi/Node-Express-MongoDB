...
router.get("/usertodos", async (req, res) => {
  try {
    const data = await User.find().select({ password: 0 }).populate("todos");
    res
      .status(200)
      .json({ message: "Todos fetch is successful!", result: data });
  } catch (err) {
    console.log(err);
    res.status(500).json({ error: "Todos fetch is failed!" });
  }
});
